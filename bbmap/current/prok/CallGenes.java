...
//			outstream.println(arg+", "+a+", "+b);
	...
	private void checkFileExistence(){
		...
		if(!Tools.testOutputFiles(overwrite, append, false, outGff, outAmino, out16S, out18S)){
			outstream.println((outGff==null)+", "+outGff);
			throw new RuntimeException("\n\noverwrite="+overwrite+"; Can't write to output files "+outGff+", "+outAmino+", "+out16S+", "+out18S+"\n");
		}
	...
	void process(Timer t){
		...
		if(call16S){outstream.println("16S Out:              \t "+Tools.padLeft(r16SOut, 12));}
		if(call18S){outstream.println("18S Out:              \t "+Tools.padLeft(r18SOut, 12));}
		if(call23S){outstream.println("23S Out:              \t "+Tools.padLeft(r23SOut, 12));}
		if(call5S){outstream.println("5S Out:               \t "+Tools.padLeft(r5SOut, 12));}
		if(calltRNA){outstream.println("tRNA Out:             \t "+Tools.padLeft(tRNAOut, 12));}
		...
		if(call16S){
			outstream.println();
			outstream.println("Called 16S Stats:");
			outstream.print(st16s);
		}
		if(call23S){
			outstream.println();
			outstream.println("Called 23S Stats:");
			outstream.print(st23s);
		}
		if(call5S){
			outstream.println();
			outstream.println("Called 5S Stats:");
			outstream.print(st5s);
		}
		if(call18S){
			outstream.println();
			outstream.println("Called 18S Stats:");
			outstream.print(st18s);
		}
